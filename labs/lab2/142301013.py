...
class YantraCollector:
    ...
    def ucs(self):
        ...
        """
        frontier = [[self.start, 0]]  # (position, cost)
        explored = []
            ...
            for neighbor in self.get_neighbors(current):
                ...
                for i in range(len(frontier)):
                    if frontier[i][0] == neighbor:
                        is_in_frontier = True
                        if frontier[i][1] > cost + move_cost:
                            frontier[i][1] = cost + move_cost
                            break
                
                if neighbor not in explored and not is_in_frontier:
                    frontier.insert(0, [neighbor, cost + move_cost])
    ...
    def heuristic(self, position):
        ...
        if grid[position[0]][position[1]] == 'T':
            return TRAP_COST
        if abs(position[0] - self.revealed_yantra[0]) == 0  or abs(position[1] - self.revealed_yantra[1])==0:
            # max(|x - x0| , |y - y0|) * log(1+10*c)
            # i used log so that the cost is not too high
            return max(abs(position[0] - self.revealed_yantra[0]), abs(position[1] - self.revealed_yantra[1]))*math.log1p(10*self.cost_map[position])
        else:
            # sqrt(|x - x0| * |y - y0|) * log(1+10*c)
            return pow(abs(position[0] - self.revealed_yantra[0])*abs(position[1] - self.revealed_yantra[1]), 0.5)*math.log1p(self.cost_map[position]*10)
    ...
    def gbfs(self):
        ...
        """
        frontier = [[self.start, self.heuristic(self.start), 0]]  # (position, heuristic, cost)
        frontier = merge_sort(frontier)
        explored = []

        while frontier:
            frontier=merge_sort(frontier)
            current, h_cost, cost = frontier.pop(0)
            ...
            for neighbor in self.get_neighbors(current):
                ...
                if neighbor not in explored and not is_in_frontier:
                    frontier.insert(0, [neighbor, self.heuristic(neighbor) , cost + move_cost])
    ...
    def a_star(self):
        ...
        """
        frontier = [[self.start, self.heuristic(self.start)+0, 0]]  # (position, heuristic+cost, cost)
        frontier = merge_sort(frontier)
        explored = []

        while frontier:
            frontier=merge_sort(frontier)
            current, g_cost, cost = frontier.pop(0)
            ...
            for neighbor in self.get_neighbors(current):
                ...
                if neighbor not in explored and not is_in_frontier:
                    frontier.insert(0, [neighbor, self.heuristic(neighbor)+move_cost , cost + move_cost])
    ...
    def solve(self, strategy):
        ...
        while self.revealed_yantra:
            if strategy == "UCS":
                ...
                fn, en, cost = self.gbfs()
            elif strategy == "A*":
                fn, en, cost = self.a_star()
            else:
                return None
